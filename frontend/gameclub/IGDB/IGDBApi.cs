...
namespace IGDB
{
  ...
  public interface IGDBApi
  {
    ...
    string ClientId { get; set; }
        ...
        [Post("/{endpoint}")]
        Task<List<T>> QueryAsync<T>([Path] string endpoint, [Body] string query = null);  //Added to help with stingconent conversion
    ...
    public sealed class IGDBClient
    {
            ...
            var api = new RestClient("https://api.igdb.com/v4", async (request, cancellationToken) =>
      {
        ...
        public async Task<List<T>> QueryAsync<T>(string endpoint, String query = null)
        {
            try
            {
